...
* **`dataset/train/`**: 모델 학습에 사용되는 데이터셋
    * `images/`: 원본 이미지
    * `images_augmented/`: `augment_dataset.py`를 통해 색상이 변경된 증강 이미지
    * `labels/`: YOLO 형식의 포트홀 라벨 파일 (`.txt` 형식)
    * `annotations.json`: 원본 데이터에 대한 COCO 형식의 주석 파일
* **`dataset/val/`**: 모델 검증에 사용되는 데이터셋
    * `images/`: 원본 이미지
    * `images_augmented/`: `augment_dataset.py`를 통해 색상이 변경된 증강 이미지
    * `labels/`: YOLO 형식의 포트홀 라벨 파일 (`.txt` 형식)
    * `annotations.json`: 원본 데이터에 대한 COCO 형식의 주석 파일
* **`dataset/test/`**: 최종 모델 성능 테스트에 사용되는 데이터셋
    * `images/`: 테스트용 원본 이미지
    * `labels/`: YOLO 형식의 포트홀 라벨 파일 (`.txt` 형식)
